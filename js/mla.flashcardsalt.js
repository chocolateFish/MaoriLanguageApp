...
mla.flashcards = (function () {
    ...
    lessonScreen = {
        main_html : String()
            ...
            + '<div id="theCard">'
                + '<h1></h1>'
                + '<img src="">'
                + '<audio src="" type=\"audio/wav\" autoplay=\"autoplay\" >Your browser does not support the audio element.</audio>'
                + '<p></p>'  
            + '</div><br>'
            + '<p id="deckLocation"></p>'
    ...
    Card = {
        ...
        engSide: function ($h, $p, $img, $audio) {
            $h.show();
            $h.text(this.myWord.getEng);
            $img.hide();
            $audio.hide();
            $p.hide();
        },
/*        maoriText: function (imgBoolean, $h1, $audio,) {
...
*/
        print: function (theImgBoolean, $theDiv) {
            var imgBoolean, $h, $p, $img, $audio;
            imgBoolean = theImgBoolean;
            $h = $theDiv.find('h1');
            $p = $theDiv.find('p');
            $img = $theDiv.find('img');
            $audio = $theDiv.find('audio');
            if (this.language === "eng") {
                $theDiv.html(this.engSide($h, $p, $img, $audio));
            } //else{
